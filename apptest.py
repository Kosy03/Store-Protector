...
MAX_LOST_FRAMES   = 30
# [설명] 사람이 연속으로 이 프레임 수만큼 화면에 안 보이면 "퇴장"으로 간주(도난 판정의 트리거)
FRAME_SKIP        = 2
...
prev_person_boxes = {}
# [설명] 직전 프레임에서의 (정규화된) 사람 ID -> 박스 위치. 새로운 ID가 나타났을 때 가까운
#        직전 위치를 찾아 같은 사람인지 비교(별칭/alias 매칭)하는 재료로 쓰임
id_alias          = {}
...
# [설명] 사람 ID별로 "연속으로 안 보인 프레임 수"를 세는 카운터. MAX_LOST_FRAMES에 도달하면
#        "퇴장했다"고 판단해 도난 여부를 검사하는 트리거가 됨
    ...
    while cap_a.isOpened() and st.session_state.running:
        ...
        if person_res.boxes.id is not None:
            for box,tid in zip(person_res.boxes, person_res.boxes.id):
                ...
                # [설명] 이 tid가 예전에 다른 ID의 "분신"으로 등록된 적이 있다면, 그 원래(정규) ID를 사용
                if tid not in id_alias and tid not in prev_person_boxes:
                    # [설명] 이번 프레임에 "완전히 새로운" tid가 등장한 경우(ByteTrack이 가려짐 등으로
                    #        ID를 새로 발급한 상황일 가능성)에만 아래 별칭 매칭을 시도
                    for pp,pb in prev_person_boxes.items():
                        ...
                        if abs(cx-pcx)<ALIAS_DIST and abs(cy-pcy)<ALIAS_DIST:
                            # [설명] 직전 프레임의 어떤 사람 위치와 ALIAS_DIST 픽셀 이내로 가까우면
                            #        "같은 사람이 ID만 바뀐 것"이라 간주하고 새 ID를 그 사람의 정규 ID로 연결
                            ...
                            for k in list(accumulated_time):
                                if k[0]==pp and (root,k[1]) not in accumulated_time:
                                    ...
                                    # [설명] ID가 바뀌면서 그동안 쌓아온 픽업 누적시간 기록까지 끊기지
                                    #        않도록, 이전 ID(pp) 기준 기록을 새 정규 ID(root) 기준으로 복사 이전
                            break
                ...
                # [설명] 이번 프레임에 보였으므로 "사라진 프레임 카운터"를 0으로 리셋
        ...
        for pid in prev_tracked_pids - current_pids:
            ...
            if lost_counter[pid] >= MAX_LOST_FRAMES:
                # [설명] 단순히 잠깐 가려진 게 아니라 충분히 오래(MAX_LOST_FRAMES) 안 보였다면
                #        진짜로 매장을 떠났다고 판단하고 아래에서 도난 여부를 검사
        ...
        # [설명] 이번에 보인 사람들(current_pids) + 아직 MAX_LOST_FRAMES에 도달하지 않아
        #        "유예 기간 중"인 사라진 사람들을 합쳐서 다음 프레임 비교 기준으로 넘김
        #        (바로 제거하지 않고 카운터가 다 찰 때까지는 계속 추적 후보로 남겨둠)
        ...
        for tid,pbox in tracked_persons:
            ...
            if in_zone:
                ...
                if (stay>=CHECKOUT_MIN_STAY and tid not in checkout_done
                        and not b_triggered):
